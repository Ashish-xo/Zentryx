  ...
  return (
    <div className="fixed inset-0 z-[250] flex items-center justify-center bg-frontier-deep/95 backdrop-blur-md"
      ...
      <div className="w-full max-w-2xl glass-frontier border border-frontier-indigo/30 p-8 m-4" onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-white font-black uppercase tracking-widest text-sm">Weather Forecast</h3>
        ...
        <div id="forecast-grid" className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-3 mb-6">
          {[1,2,3,4,5,6].map(i => (
            <div key={i} className="glass-frontier p-3 text-center border border-frontier-indigo/10">
              <div className="text-[9px] uppercase tracking-widest text-frontier-indigo/60 mb-1">Day {i}</div>
        ...
        <div className="text-frontier-text text-[10px] uppercase tracking-widest text-center">
          Live weather data available on the dashboard weather card.
        </div>
      </div>
    </div>
  );
    ...
    <div className="fixed inset-0 z-[250] flex items-center justify-center bg-frontier-deep/95 backdrop-blur-md"
      ...
      <div className="w-full max-w-lg glass-frontier border border-frontier-indigo/30 p-8 m-4" onClick={e => e.stopPropagation()}>
        ...
        <div className="space-y-4">
          ...
          <button onClick={() => { addLog('Budget estimate requested.'); setBudgetModal(false); }}
            className="w-full bg-frontier-lime text-frontier-deep py-3 text-[10px] font-black uppercase tracking-widest hover:brightness-110 transition-all">
            Estimate Budget
          </button>
          <div id="budget-loading" className="hidden text-center text-frontier-lime/60 text-[10px] animate-pulse">Calculating optimum...</div>
          <div id="budget-grid" className="hidden grid grid-cols-1 gap-3"></div>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center bg-frontier-deep/90 backdrop-blur-sm"
      ...
      <div className="w-full max-w-lg glass-frontier border border-frontier-indigo/30 p-8 m-4" onClick={e => e.stopPropagation()}>
        ...
        <div id="ask-modal-content" className="max-h-64 overflow-y-auto mb-4 space-y-2 text-[11px] text-frontier-text uppercase tracking-wider"></div>
        <div className="flex gap-2">
          <input id="ask-input" type="text" placeholder="Ask anything..."
          ...
          <button onClick={() => { /* confirmAsk */ }} className="bg-frontier-lime text-frontier-deep px-4 py-2 text-[10px] font-black uppercase tracking-widest hover:brightness-110">Ask</button>
        </div>
      </div>
    </div>
  );
